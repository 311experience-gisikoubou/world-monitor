...
- **`handoff`**：作業を別のチャット・別のAI・別のセッションへ引き継ぐためのProject Context固定と、通常ターンの短い暗黙継続が別Projectへ流れないためのturn-start整合確認。人間判断の正本はHuman Decision Syncに一本化し、local repository経路の`project-working-memory.mjs`はapproved reference・Current Goal・Next Stepの短期キャッシュとartifact recallだけを担当する。focus内のdecision IDはcanonical confirmed decisionへ照合し、古い判断のローカルコピーを権威として再利用しない。
