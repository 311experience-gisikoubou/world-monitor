...
- ユーザーが「マージ手前まで」を明示した場合、完了目標は`PRE_MERGE_READY`とする。必要修正、EARLY / MILESTONE CHECK、test-gate PASS、FINAL REALITY CHECK PASS、commit、通常push、Draft PR、final-pr-audit PASS、exact PR / exact HEAD確認を完了し、merge承認だけを残す。
...
- `STAGED_REALITY_GATE_REQUIRED=YES`: implementation work must not move from the first meaningful slice to deeper detail, from a structural milestone to completion, or from test-gate to final PR audit without the applicable EARLY / MILESTONE / FINAL REALITY check. Use `.agents/skills/test-gate/staged-reality-gate.mjs`, prefer objective task-type evidence, fail closed on identity/authority/state mismatch, and reuse still-valid evidence instead of duplicating tests.
